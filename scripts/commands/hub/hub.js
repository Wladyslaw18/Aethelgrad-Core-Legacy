...
export class HubCommand extends BaseCommand {
    constructor() {
        super({
            ...
            department: null // Anyone can use hub command
        });
    }
    ...
    execute(sender, args) {
        ...
        const lastUse = hubCooldowns.get(sender.id) ?? 0;
        ...
        const inCombat = ticksSinceDamage(sender) < 300; // 300 ticks = 15 seconds
        if (inCombat) {
            return { 
                status: CustomCommandStatus.Failure, 
        ...
        system.run(() => {
            try {
                ...
                sender.sendMessage("§cTeleport failed: " + teleportError.message);
            }
        });
